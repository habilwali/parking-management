...
async function getNightSessions(
  db: Db,
  page: number = 1,
  pageSize: number = 20,
  paymentFilter?: "paid" | "unpaid",
  search?: string,
): Promise<{ sessions: NightSession[]; total: number }> {
  ...
  if (paymentFilter === "paid") {
    // Show records where paidAmount >= price (fully paid)
    query.$expr = {
      $gte: [{ $ifNull: ["$paidAmount", 0] }, "$price"],
    };
  } else if (paymentFilter === "unpaid") {
    // Show records where paidAmount < price or paidAmount doesn't exist
    query.$expr = {
  ...
  const [sessions, total] = await Promise.all([
    db
      ...
      .limit(pageSize)
      .toArray(),
    db.collection("night_sessions").countDocuments(query),
  ]);
...
export default async function NightParkingPage({
  searchParams,
}: {
  ...
  const db = client.db();
  const { sessions, total } = await getNightSessions(db, currentPage, pageSize, paymentFilter, searchQuery);
    ...
    <div className="bg-muted/20 px-4 py-6 sm:py-10">
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 rounded-2xl border bg-card p-5 shadow-sm sm:p-8">
        <header className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-semibold text-foreground sm:text-3xl">
              All Night Parking Records
            </h1>
            <p className="text-sm text-muted-foreground">
              Complete list of all night parking sessions
            </p>
          </div>
        ...
        <div className="flex items-center justify-between gap-2 mb-2">
          <VehicleSearch baseUrl="/night-parking" />
          <PaymentFilter currentFilter={paymentFilter} baseUrl="/night-parking" />
        </div>

        {sessions.length === 0 ? (
          <div className="rounded-xl border border-dashed bg-background p-8 text-center text-sm text-muted-foreground">
            No night parking sessions found.
          </div>
        ) : (
          <>
